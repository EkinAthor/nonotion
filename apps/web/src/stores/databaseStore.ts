...
interface DatabaseState {
  ...
  updateRowProperties: (rowId: string, properties: Record<string, PropertyValue>) => Promise<void>;
...
export const useDatabaseStore = create<DatabaseState>((set, get) => ({
  ...
  updateSchema: async (input) => {
    const { activeDatabaseId } = get();
    if (!activeDatabaseId) return null;
  ...
  updateRowProperties: async (rowId, properties) => {
    try {
      await databaseApi.updateProperties(rowId, { properties });
