/**
 * Seed script: loads demo content into the backend storage (SQLite or PostgreSQL).
 *
 * Usage:
 *   pnpm --filter @nonotion/api seed:demo
 *
 * The demo data mirrors apps/web/src/api/demo-data.ts (the browser-only demo).
 * All content is assigned to admin@example.com (created if missing).
 * The script is idempotent — existing entities are skipped.
 */
...
import type { Page, Block, PagePermission } from '@nonotion/shared';
...
// ─── Page IDs (stable, matching frontend demo-data.ts) ───────────────────────
...
function makeBookRow(idx: number, book: BookData, ownerId: string): Page {
  return {
    id: ROW_IDS[idx],
    title: book.title,
    type: 'document',
...
function createPages(ownerId: string): Page[] {
  const bookRows = books.map((b, i) => makeBookRow(i, b, ownerId));
...
// ─── Migrations (copied from index.ts) ───────────────────────────────────────
...
async function seed(): Promise<void> {
  ...
  // ── Pages ──────────────────────────────────────────────────────────────────
  const pages = createPages(ownerId);
  let pagesCreated = 0;
  let pagesSkipped = 0;
  ...
  for (const block of blocks) {
    const existing = await storage.getBlock(block.id);
    if (existing) {
      blocksSkipped++;
    } else {
  ...
  console.log(`Blocks: ${blocksCreated} created, ${blocksSkipped} skipped (already exist)`);

  // ── Permissions ────────────────────────────────────────────────────────────
  const rootPages = [PG_BOOKS, PG_SHOWCASE];
  let permsCreated = 0;
  let permsSkipped = 0;

  for (const pageId of rootPages) {
    const existing = await userStorage.getPermission(pageId, ownerId);
    if (existing) {
      permsSkipped++;
    } else {
  ...
  console.log(`Permissions: ${permsCreated} created, ${permsSkipped} skipped (already exist)`);
